...
export class NebulaContext {
 ...
 public static async  init_context(model: NebulaModel) : NebulaContext {
    const ctx = await invoke('plugin:nebula|model_init_context', {
      model: model.model,
      contextOptions: {},
    });
  ...
  public async predict(max_len: i32) {
    const unlisten = await listen('nebula-predict', (event) => {
