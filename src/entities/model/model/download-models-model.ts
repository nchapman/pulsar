...
const $multiProgress = createStore(0);
export const setProgress = createEvent<number>();
export const setMmpProgress = createEvent<number>();
$progress.on(setProgress, (_, progress) => progress);
$multiProgress.on(setMmpProgress, (_, progress) => progress);

const $total = createStore(0);
const $multiTotal = createStore(0);
export const setTotal = createEvent<number>();
export const setMmpTotal = createEvent<number>();
...
export const $percentMmp = combine($multiProgress, $multiTotal, (progress, total) =>
  !total ? 0 : (progress / total) * 100
);
...
export const $downloadedMmp = combine(
  $multiProgress,
  $multiTotal,
  (progress, total) => total > 0 && progress === total
);
export const $downloadingMmp = combine(
  $multiProgress,
  $multiTotal,
  (progress, total) => progress > 0 && progress < total
);
