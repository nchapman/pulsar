...
import { StoreAgent } from '@/widgets/agents/types/agent.types.ts';
...
const [$agents] = getState<StoreAgent[]>(agentsMock);

// derived
const $agentsMap = $agents.map((agents) =>
  agents.reduce<Record<Id, StoreAgent>>((acc, agent) => ({ ...acc, [agent.id]: agent }), {})
);
